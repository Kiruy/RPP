...
namespace Ship2
{
    public partial class FormMapWithSetShips : Form
    {
        ...
        private void AddPlane(DrowingClassicShip mpln)
        {
            ...
            try
            {
                DrowingShip ship = new DrowingShip(mpln);
        ...
        private void ButtonShowStorage_Click(object sender, EventArgs e)
        {
            ...
            pictureBoxShip.Image = _mapsCollection[listBoxMaps.SelectedItem?.ToString() ?? string.Empty].ShowSet();
            _logger.LogInformation($"Вывод набора");
        }
        /// <summary>
        /// Вывод карты
        /// </summary>
        ...
        private void ButtonShowOnMap_Click(object sender, EventArgs e)
        {
            ...
            _logger.LogInformation($"Вывод карты c объектом");
        }
        ...
        private void ButtonMove_Click(object sender, EventArgs e)
        {
            ...
            switch (name)
            {
                case "buttonUp":
                    dir = Direction.Up;
                    _logger.LogInformation($"Перемещение вверх");
                    break;
                case "buttonDown":
                    dir = Direction.Down;
                    _logger.LogInformation($"Перемещение вниз");
                    break;
                case "buttonLeft":
                    dir = Direction.Left;
                    _logger.LogInformation($"Перемещение влево");
                    break;
                case "buttonRight":
                    dir = Direction.Right;
                    _logger.LogInformation($"Перемещение в право");
                    break;
            }
        ...
        {
            pictureBoxShip.Image = _mapsCollection[listBoxMaps.SelectedItem?.ToString() ?? string.Empty].ShowSet();
            _logger.LogInformation($"Выбор карты {listBoxMaps.SelectedItem?.ToString() ?? string.Empty}");
        }
        ...
        private void SaveToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (saveFileDialog.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    ...
                    _logger.LogInformation($"Сохранение файла {saveFileDialog.FileName}");
                }
                catch (Exception ex)
                {
            ...
            if (openFileDialog.ShowDialog() == DialogResult.OK)
            {
                try
                {
                   ...
                   MessageBoxIcon.Information);
                    ReloadMaps();
                    pictureBoxShip.Image = _mapsCollection[listBoxMaps.SelectedItem?.ToString() ?? string.Empty].ShowSet();
                    _logger.LogInformation($"Загрузка файла {openFileDialog.FileName}");
                }
                catch (Exception ex)
                {
